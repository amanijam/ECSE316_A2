...
def naiveDFT(arr):
    ...
    for i in range(len(arr)):
        transform[i] = np.sum(
            ...
            arr
            * np.exp((-2j * np.pi) / len(arr) * i * np.arange(len(arr)))
        )
...
def FFT(arr):
    ...
    if n < smallSize:  # decided threshold
        ...
        exp = np.exp(
            (-2j * np.pi * k) / n
        )  # calculates exponential terms for each k value
...
def plottingMode():
    ...
    )  # Naive method error bars
    plt.plot(size_list, fft_list, "orange", label="FFT", marker="o")  # FFT method line
    plt.errorbar(
        size_list, fft_list, yerr=np.std(fft_list), ecolor="orange", capsize=8
    )  # FFt method error bars
    plt.legend()
    plt.show()
